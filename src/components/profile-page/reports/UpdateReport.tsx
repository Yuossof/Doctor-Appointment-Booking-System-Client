...
const UpdateReport = () => {
    ...
    const [formData, setFormData] = useState({
        type: showBox?.doc?.type || '',
        desc: showBox?.doc?.desc || '',
    })
    ...
    const formAction = async () => {
        ...
        try {
            ...
            setPending(false);
            showBox?.setShowBoxUpdate(false)
    ...
    return (
        <>
            {showBox?.showBoxUpdate && (
                <div
                    onClick={() => showBox?.setShowBoxUpdate(false)}
                    ...
                    <div className='w-full flex justify-center items-center'>
                        ...
                        >
                            <X onClick={() => showBox?.setShowBoxUpdate(false)} className='absolute top-2 right-2 cursor-pointer' size={18} />
                            <div className='w-full flex flex-col gap-3 mt-4'>
                                <div>
                                    <Input className='mb-1' onChange={handleChange} name='type' defaultValue={showBox?.doc?.type} type='text' placeholder='Report Name' />
                                    {errors?.type && <span className="text-red-600">{errors.type[0]}</span>}
                                </div>
                                <div>
                                    <Textarea className='mb-1' onChange={handleChange} name='desc' defaultValue={showBox?.doc?.desc} placeholder='Report Description' />
